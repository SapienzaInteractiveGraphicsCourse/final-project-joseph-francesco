...
EnnemiesHolder.prototype.rotateEnnemies = function(){
  for (var i=0; i<this.ennemiesInUse.length; i++){
    ...
    var diffPos = airplane.position.clone().sub(ennemy.mesh.position.clone());
...
CoinsHolder.prototype.rotateCoins = function(){
  for (var i=0; i<this.coinsInUse.length; i++){
    ...
    var diffPos = airplane.position.clone().sub(coin.mesh.position.clone());
    var d = diffPos.length();
...
function loop(){
  ...
  if (game.status=="playing"){
    ...
    game.speed *= .99;
    airplane.rotation.z += (-Math.PI/2 - airplane.rotation.z)*.0002*deltaTime;
    airplane.rotation.x += 0.0003*deltaTime;
    game.planeFallSpeed *= 1.05;
    airplane.position.y -= game.planeFallSpeed*deltaTime;

    if (airplane.position.y <-200){
      showReplay();
  ...
  // airplane.rotation.x +=.2 + game.planeSpeed * deltaTime*.005;
